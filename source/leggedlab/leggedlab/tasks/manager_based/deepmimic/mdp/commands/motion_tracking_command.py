...
class ReferenceState:
    ...
    def __init__(self, num_envs: int, num_bodies: int, num_joints: int, device: str):
        ...
        self.body_pos = torch.zeros((num_envs, num_bodies, 3), device=device, dtype=torch.float32)
        ...
        self.key_points = torch.zeros((num_envs, num_bodies, 3, 3), device=device, dtype=torch.float32)
...
class MotionTrackingCommand(CommandTerm):
    ...
    def command(self) -> torch.Tensor:
        ...
        return torch.cat(
            [
                ...
                self.joint_pos[:, self.dataset_to_robot_joint_indices][indices].flatten(start_dim=1),
                self.joint_vel[:, self.dataset_to_robot_joint_indices][indices].flatten(start_dim=1),
            ],
            dim=-1,
        )
    ...
    def body_pos_w_error(self) -> torch.Tensor:
        return self.ref.body_pos_w - self.robot.data.body_link_pos_w[:, self.robot_body_indices]

    @property
    def body_height_error(self) -> torch.Tensor:
        return self.body_pos_w_error[..., 2]

    @property
    def root_height_error(self) -> torch.Tensor:
        return self.ref.body_pos_w[:, self.root_link_ids, 2] - self.robot.data.root_link_pos_w[:, 2].unsqueeze(1)

    @property
    def tracking_body_pos(self) -> torch.Tensor:
        return self.ref.body_pos

    @property
    def tracking_body_quat(self) -> torch.Tensor:
        return self.ref.body_quat

    @property
    def body_lin_vel_error(self) -> torch.Tensor:
        body_lin_vel_b = math_utils.quat_apply_inverse(
            self.robot.data.body_link_quat_w, self.robot.data.body_link_lin_vel_w
        )
        return self.ref.body_vel - body_lin_vel_b[:, self.robot_body_indices]

    @property
    def body_ang_vel_error(self) -> torch.Tensor:
        body_ang_vel_b = math_utils.quat_apply_inverse(
            self.robot.data.body_link_quat_w, self.robot.data.body_link_ang_vel_w
        )
        return self.ref.body_ang_vel - body_ang_vel_b[:, self.robot_body_indices]

    @property
    def joint_pos_error(self) -> torch.Tensor:
        return self.ref.joint_pos - self.robot.data.joint_pos[:, self.robot_joint_indices]

    @property
    def joint_vel_error(self) -> torch.Tensor:
        return self.ref.joint_vel - self.robot.data.joint_vel[:, self.robot_joint_indices]

    @property
    def tracking_key_points_w(self) -> torch.Tensor:
        return self.ref.key_points_w

    @property
    def tracking_key_points(self) -> torch.Tensor:
        return self.ref.key_points
    ...
    def _update_ref_state(self, env_ids: Sequence[int] | None = None):
        ...
        # Update body positions in local frame
        self.ref.body_pos[env_ids] = self.body_pos_xyz[index]
        ...
        self.ref.key_points[env_ids] = self.keypoints[index]

        # Update world frame positions based on configuration
        if self.cfg.use_world_frame:
            self.ref.body_pos_w[env_ids] = self.body_pos_xyz_w[index] + self._env.scene.env_origins[env_ids].unsqueeze(
                1
            )
            ...
            # Transform to world frame using current root position and yaw
            root_link_pos_w = self.robot.data.root_link_pos_w[env_ids, :].clone()
            root_link_pos_w[:, 2] = self.body_pos_xyz_w[index, self.root_link_ids, 2].clone()
            root_yaw_quat = math_utils.yaw_quat(self.body_quat_wxyz[index, self.root_link_ids, :])

            self.ref.body_pos_w[env_ids] = math_utils.quat_apply(
                root_yaw_quat.unsqueeze(1).expand(-1, self.body_pos_xyz.shape[1], -1), self.body_pos_xyz[index]
            ) + root_link_pos_w.unsqueeze(1)

            self.ref.key_points_w[env_ids] = math_utils.quat_apply(
                root_yaw_quat.unsqueeze(1)
                .unsqueeze(1)
                .expand(-1, self.keypoints.shape[1], self.keypoints.shape[2], -1),
    ...
    def _debug_vis_callback(self, event):
        ...
        body_positions_w = self.ref.body_pos_w[:, self.tracking_body_ids, :]
        body_orientations = self.ref.body_quat[:, self.tracking_body_ids, :]
        self.tracking_body_frame_visualizer.visualize(
            translations=body_positions_w.reshape(-1, 3),
            orientations=body_orientations.reshape(-1, 4),
        )

        # Visualize key points
        key_points_w = self.ref.key_points_w[:, self.tracking_body_ids, :, :]
        self.key_points_visualizer.visualize(
            translations=key_points_w.reshape(-1, 3),
        )
    ...
    def _load_dataset(self):
        ...
        for dataset_path in self.cfg.dataset_path:
            ...
            if not hasattr(self, "dof_names") and not hasattr(self, "body_names"):
                self.body_names = dataset["body_names"].tolist()
                ...
                self.joint_names = dataset["dof_names"].tolist()
        ...
        self.body_pos_xyz = math_utils.quat_apply_inverse(
            math_utils.yaw_quat(self.body_quat_wxyz[:, self.root_link_ids, :]).expand(
                -1, self.body_pos_xyz_w.shape[1], -1
            ),
            self.body_pos_xyz_w - self.body_pos_xyz_w[:, self.root_link_ids, :],
        )
        keypoints_b = self.cfg.side_length * torch.eye(
            3, device=self.body_pos_xyz_w.device, dtype=self.body_pos_xyz_w.dtype
        )
        keypoints_b = (
            keypoints_b.unsqueeze(0)
            .unsqueeze(0)
            .expand(self.body_pos_xyz_w.shape[0], self.body_quat_wxyz.shape[1], -1, -1)
        )
        self.keypoints_w = math_utils.quat_apply(
            self.body_quat_wxyz.unsqueeze(2).expand(-1, -1, keypoints_b.shape[2], -1), keypoints_b
        ) + self.body_pos_xyz_w.unsqueeze(2)
        self.keypoints = math_utils.quat_apply_inverse(
            math_utils.yaw_quat(self.body_quat_wxyz[:, self.root_link_ids, :].unsqueeze(2)).expand(
                -1, self.body_pos_xyz_w.shape[1], self.keypoints_w.shape[2], -1
            ),
