...
# Pre-defined configs
##
from leggedlab.terrains import ROUGH_TERRAINS_CFG  # isort: skip
...
class RewardsCfg:
    ...
    body_lin_vel_z_l2 = RewTerm(
        func=mdp.body_lin_vel_z_l2, weight=-1.0, params={"asset_cfg": SceneEntityCfg("robot", body_names="base")}
    )
...
class TerminationsCfg:
    ...
    base_contact = DoneTerm(
        func=mdp.illegal_contact_with_immune,  # type: ignore
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names="base"),
            "threshold": 1.0,
            "immune_probability": 0.0,
...
class CurriculumCfg:
    ...
    terrain_levels = CurrTerm(
        func=mdp.terrain_levels_vel,  # type: ignore
        params={"move_up_threshold": 0.5, "move_down_threshold": 0.4},
    )

    command_levels = CurrTerm(
        func=mdp.command_levels_vel,  # type: ignore
        params={"delta_vel": [0.1, 0.1, 0.1], "max_vel_range": [(-1.0, 3.0), (-0.5, 0.5), (-3.0, 3.0)]},
    )
