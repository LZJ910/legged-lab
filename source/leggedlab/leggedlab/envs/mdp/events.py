...
def apply_external_force_torque_stochastic(
    ...
    force_range_tensor = torch.tensor(force_range_list, device=asset.device)
    forces = math_utils.sample_uniform(force_range_tensor[:, 0], force_range_tensor[:, 1], size, asset.device)
    torque_range_list = [torque_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z"]]
    torque_range_tensor = torch.tensor(torque_range_list, device=asset.device)
    torques = math_utils.sample_uniform(torque_range_tensor[:, 0], torque_range_tensor[:, 1], size, asset.device)
